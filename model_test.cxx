...
TEST_CASE("Model:: paddle_to"){
    ...
    m.ball.radius = 5;
    m.ball.center = {1,1};
    CHECK_FALSE(m.ball.center.x == 90);
    CHECK_FALSE(m.ball.center.y == 994);
    m.paddle_to(40);
    CHECK(m.paddle.x == 40);
    CHECK(m.ball.center.x == 90);
    CHECK(m.ball.center.y == 994);
    m.ball.live = true;
    m.paddle_to(200);
    CHECK(m.paddle.x == 200);
    CHECK(m.ball.center.x == 90);
    CHECK(m.ball.center.y == 994);

}
TEST_CASE("Modell:: on_frame ball isn't live"){
    Model m(config);
    double const dt = 0.5;
    m.bricks.clear();
    m.paddle = {50, 400, 600, 50};
    m.ball.velocity = {70, 70};
    m.ball.center.x = 300;
    m.ball.center.y = 50;
    m.on_frame(dt);
    CHECK(m.ball.center.x == 300);
    CHECK(m.ball.center.y == 50);
    CHECK(m.ball.velocity.height == 70);
    CHECK(m.ball.velocity.width == 70);

}
TEST_CASE("bounce off side wall and top"){
    Model m(config);
...
TEST_CASE(" hit bottom"){
    ...
    m.paddle = {50, 400, 600, 50};
    m.ball.live = true;
    m.ball.velocity = {0, 70};
    m.ball.center.x = 300;
    m.ball.center.y = 100;
    m.ball.radius = 5;
    m.on_frame(dt);
    CHECK(m.ball.center.x == 300);
    CHECK(m.ball.center.y == 135);
    CHECK(m.ball.velocity.height == 70);
    CHECK(m.ball.velocity.width == 0);
    m.ball.center.y = m.config.scene_dims.height - m.ball.radius - 1;
    double const new_dt = 1;
    m.on_frame(new_dt);
    CHECK(m.ball.center.x == 350);
    CHECK(m.ball.center.y == 394);
    CHECK(m.ball.live == false);
